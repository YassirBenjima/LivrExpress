...
final class Version20260421121500 extends AbstractMigration
{
    ...
    {
        return 'Seed first two whatsapp templates and enforce fixed default/active statuses';
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($count === 0) {
            $this->addSql(
                ...
                ('Pour client', 'Bonjour @name, votre colis @product est en cours de livraison a @address. Pour toute information, contactez le livreur au @numLivreur.', 'active', '{$now}', '{$now}')"
            );
        ...
        $this->addSql(
            ...
            AND id <> (SELECT id FROM (SELECT id FROM whatsapp_template ORDER BY id ASC LIMIT 1) first_template)"
        );
        $this->addSql(
            "UPDATE whatsapp_template SET status = 'default'
            WHERE id = (SELECT id FROM (SELECT id FROM whatsapp_template ORDER BY id ASC LIMIT 1) first_template)"
        ...
        $this->addSql(
            "UPDATE whatsapp_template SET status = 'active'
            WHERE id = (SELECT id FROM (SELECT id FROM whatsapp_template ORDER BY id ASC LIMIT 1 OFFSET 1) second_template)"
        );
    }
